...
function CameraModule(dotNetObject /* object */, videoElement /* ref */) {
  //const videoElement = document.getElementById(videoElementId);
  const cameraPhoto = new JslibHtml5CameraPhoto.default(videoElement);

  return {
    startCamera: () => {
      cameraPhoto.startCameraMaxResolution(FACING_MODES.ENVIRONMENT, {}).then((stream) => {
        //console.log('Camera started.');
        dotNetObject.invokeMethodAsync('OnCameraResponse', 'SUCCESS', 'Camera started.');
      }).catch(err => {
        console.error('Camera not started!', err);
